...
import json
import argparse
import unicodedata # NEW: For robust text handling
...
from Day_21_E import init_cache_db, init_analytics_db, init_leads_db # ADDED init_leads_db
...
def _normalize_url(url):
    ...
    # FIX: Ensure query param cleaning handles both str and list values safely
...
def fetch_page(url, render_js=False):
    ...
    try:
        ...
        if e.response.status_code in (429, 503, 404):
         ...
         logging.warning(f"Static fetch failed for {url} (General Error): {e}. Trying JS render.")
         response = None
         
    
    # 2. Dynamic Fetch (Playwright) if requested or static failed/content short
    if render_js or (response is None or text_len < RENDER_JS_THRESHOLD_WORDS):
        ...
        try:
            with sync_playwright() as p:
             ...
             # FIX: Return static content if it exists, otherwise None
             return response.text if response is not None else None, final_url 
        except Exception as e:
...
def _read_robustly(filepath):
    """Robustly reads a text file using multiple encodings and normalizes Unicode."""
    ...
    for encoding in encodings_to_try:
        try:
            ...
            logging.warning(f"Decoding failed with {encoding}. Trying next encoding.")
            continue
        except Exception as e:
            logging.error(f"Failed to read {filepath} with {encoding}: {e}")
            return None # Skip this file if other error occurs
    
    logging.error(f"Failed to read file {filepath} with all tested encodings.")
    return None
...
def extract_page_metadata(html, url):
    ...
    for tag in main_content_area.find_all(TARGET_TAGS):
        # FIX: Ensure we only exclude tags that are direct parents of the current tag
        parent_names = [p.name for p in tag.parents]
        if any(p_name in EXCLUSION_TAGS for p_name in parent_names): continue
        if tag.name in EXCLUSION_TAGS: continue # Exclude the tag itself if it's an exclusion tag (e.g., if a header is used for nav)
        ...
        # Apply robust reading/normalization logic to the extracted text
...
def chunk_text(text, metadata, chunk_size=CHUNK_SIZE, overlap=OVERLAP):
    ...
    while start < len(words):
        ...
        # FIX: Ensure a unique ID is generated robustly
        chunk_id_content = f"{metadata['path'].replace('/', '_')}_{id_counter}_{hashlib.sha1(chunk.encode('utf-8')).hexdigest()[:6]}"
        chunk_metadata = {
            'id': chunk_id_content,
            'text': chunk,
            'url': metadata['url'], 'canonical': metadata['canonical'], 'title': metadata['title'],
            # FIX: Ensure headings is stored as a JSON string for ChromaDB metadata
...
def crawl(seed_urls, max_depth, render_js_flag, sitemap_only=False, progress_bar=None):
    ...
    while queue:
        ...
        # FIX: Use canonical URL for visited check if available, but for now, use the requested URL
        ...
        # 3. Save raw HTML (optional, for debug)
...
def build_and_index_knowledge_base(max_depth, render_js_flag, sitemap_only, progress_bar=None):
    ...
    if not all_chunks: 
        ...
        return None, None # Return None for both main collection and FAQ collection
    ...
    try:
        ...
        # FIX: Ensure a clean slate for the main collection rebuild
        try:
             client.delete_collection(name=COLLECTION_NAME)
             logging.info(f"Successfully deleted existing main collection: {COLLECTION_NAME}")
        except ValueError:
             pass 
        except Exception as e:
             logging.warning(f"Warning during main collection deletion: {e}")
        ...
        metadatas = [
            {
                ...
                'title': chunk['title'],
                # 'headings' is already a JSON string from chunk_text in the fix above
        ...
        # FIX: Batch the add operation for stability, though not strictly required for this size
        batch_size = 500 
        ...
        # Build the FAQ index immediately after the main KB is built
        faq_collection = build_and_index_faq_suggestions(client)
        ...
        return collection, faq_collection # Return both collections
...
def build_and_index_faq_suggestions(client=None):
    """Builds a dedicated ChromaDB collection for the 100 FAQ suggestions."""
    ...
    try:
        ...
        # --- FIX: Ensure the collection is deleted cleanly before creation ---
        try:
            ...
            logging.warning(f"Warning during FAQ collection deletion: {e}")
        
        # Now, create the collection from scratch
        ...
        logging.error(f"FATAL: Failed to initialize/index FAQ ChromaDB: {e}")
        return None
...
def load_or_build_knowledge_base():
    ...
    # FIX: Initialize all SQLite databases safely before attempting any ChromaDB operation
    init_cache_db() 
    init_analytics_db()
    init_leads_db()
    ...
    # 1. Attempt to load existing collections
    try:
        client = chromadb.PersistentClient(path=CHROMA_DB_PATH)
        ...
        if COLLECTION_NAME in collection_names:
            main_collection = client.get_collection(name=COLLECTION_NAME, embedding_function=_EMBEDDING_FUNCTION)
        
        if FAQ_COLLECTION_NAME in collection_names:
            faq_collection = client.get_collection(name=FAQ_COLLECTION_NAME, embedding_function=_EMBEDDING_FUNCTION)

        # Check if the main collection is valid
        if main_collection and main_collection.count() > 0:
            progress_bar.empty()
            status_container.info(f"💾 Knowledge Base Loaded: {main_collection.count()} indexed chunks.", icon="⚡")
            
            # Check if the FAQ collection is valid
            if faq_collection and faq_collection.count() > 0:
                 status_container.success(f"✅ FAQ Index Loaded: {faq_collection.count()} questions.", icon="💡")
            else:
                 # If main KB loaded but FAQ is missing, rebuild ONLY the FAQ index
                 faq_collection = build_and_index_faq_suggestions(client)
                 if faq_collection:
                     status_container.success("✅ FAQ Index Built and Loaded.", icon="💡")
            
            return main_collection, faq_collection # Return both
    ...
    # 2. Run the full build sequence if loading failed
    main_collection, faq_collection = build_and_index_knowledge_base(
        max_depth=SCRAPE_MAX_DEPTH, render_js_flag=RENDER_JS, sitemap_only=False, progress_bar=progress_bar
    )
    
    if main_collection:
        status_container.success("⚡ Knowledge Base Build Complete.", icon="🚀")

    return main_collection, faq_collection
...
def main_cli():
    ...
    if args.build or args.update:
        ...
        init_cache_db() 
        init_analytics_db()
        init_leads_db() # NEW: Initialize leads database
        
        # This function now handles building BOTH collections
        build_and_index_knowledge_base(
            max_depth=args.max_depth, render_js_flag=args.render_js, sitemap_only=args.sitemap_only
        )
    else:
        parser.print_help()
        print("\nTo run the chatbot UI, use: streamlit run Day_21_D.py")


if __name__ == '__main__':
    # Checks if running via Streamlit or as a standalone script
    if 'streamlit' not in os.environ.get('COMMAND_NAME', '').lower():
        main_cli()
    else:
        # Prevents running main_cli() when Streamlit imports this file
        pass