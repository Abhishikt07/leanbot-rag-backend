...
from Day_20_E import init_cache_db, init_analytics_db, init_leads_db # ADDED init_leads_db
...
def build_and_index_knowledge_base(max_depth, render_js_flag, sitemap_only, progress_bar=None):
    ...
    try:
        ...
        # NEW FIX: Use get_or_create_collection, and if the count is > 0, clear it first.
        collection = client.get_or_create_collection(
            name=COLLECTION_NAME, metadata={"hnsw:space": "cosine"}, embedding_function=_EMBEDDING_FUNCTION
        )

        # If the collection already exists and has documents, delete them before adding new ones
        if collection.count() > 0:
            logging.info(f"Clearing {collection.count()} existing documents from collection: {COLLECTION_NAME}")
...
def main_cli():
    ...
    if args.build or args.update:
        ...
        parser.print_help()
        print("\nTo run the chatbot UI, use: streamlit run Day_20_D.py")
